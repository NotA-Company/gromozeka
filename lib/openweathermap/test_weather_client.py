...
from lib.openweathermap.models import GeocodingResult, WeatherData, CombinedWeatherResult
...
class TestOpenWeatherMapClient:
    ...
    @pytest.mark.asyncio
    async def test_get_weather_by_city_success(self, client, mock_cache, sample_geocoding_response, sample_weather_response):
    ...
    @pytest.mark.asyncio
    async def test_context_manager(self, mock_cache):
        """Test async context manager"""
        client = OpenWeatherMapClient(apiKey="test", cache=mock_cache)
        
        async with client as c:
            assert c.session is not None
        
        # Session should be closed after context exit
        # Note: We can't easily test this without mocking aiohttp
...
class TestIntegration:
    ...
    async def test_full_workflow_with_real_cache(self):
        ...
        weather_response = {
            ...
            "lon": 37.6173,
            "timezone": "Europe/Moscow",
            "current": {"dt": 1697644800, "temp": 15.5, "feels_like": 14.2, "pressure": 1013, "humidity": 65, "clouds": 40, "wind_speed": 3.5, "wind_deg": 180, "weather": [{"id": 802, "main": "Clouds", "description": "scattered clouds"}]},
            "daily": []
        }
