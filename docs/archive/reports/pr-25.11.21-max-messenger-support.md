...
Реализация комплексной поддержки Max Bot с полной клиентской библиотекой, архитектурным рефакторингом и улучшенной системой обработчиков. Это крупное обновление вводит интеграцию с новой платформой обмена сообщениями наряду с существующей поддержкой Telegram, включая параллельную обработку чатов, улучшенную обработку разметки и обширные улучшения документации.

**Ключевое изменение:** Добавление полной поддержки платформы Max Bot с клиентской библиотекой, моделями и унифицированной архитектурой обработчиков
...
### Files Modified
- [`internal/bot/handlers/`](internal/bot/handlers/) → [`internal/bot/common/handlers/`](internal/bot/common/handlers/) - Moved handlers to common directory for platform sharing
- [`internal/bot/application.py`](internal/bot/application.py) → [`internal/bot/telegram/application.py`](internal/bot/telegram/application.py) - Moved Telegram-specific application
