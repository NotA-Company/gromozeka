...
# ============================================================================
# Integration Tests - /summary Command Flow
# ============================================================================


class TestSummaryCommandFlow:
    """Test complete /summary command flow, dood!"""

    @pytest.mark.asyncio
    async def testSummaryCommandInPrivateChatWithoutArgs(
        self, summarizationHandler, mockBot, mockDatabase, mockCacheService
    ):
        """Test /summary command in private chat launches wizard, dood!"""
        summarizationHandler.injectBot(mockBot)

        mockCacheService.getChatSettings.return_value = {ChatSettingsKey.ALLOW_SUMMARY: ChatSettingsValue("true")}

        user = createMockUser(userId=456, username="testuser")
        chat = createMockChat(chatId=456, chatType=Chat.PRIVATE)
        message = createMockMessage(chatId=456, userId=456, text="/summary")
        message.chat = chat
    ...
    @pytest.mark.asyncio
    async def testSummaryCommandInGroupChat(
        self, summarizationHandler, mockBot, mockDatabase, mockCacheService, mockLlmManager
    ):
        """Test /summary command in group chat summarizes directly, dood!"""
        summarizationHandler.injectBot(mockBot)

        mockCacheService.getChatSettings.return_value = {
            ChatSettingsKey.ALLOW_SUMMARY: ChatSettingsValue("true"),
            ChatSettingsKey.SUMMARY_MODEL: ChatSettingsValue("gpt-4"),
            ChatSettingsKey.SUMMARY_FALLBACK_MODEL: ChatSettingsValue("gpt-3.5-turbo"),
            ChatSettingsKey.SUMMARY_PROMPT: ChatSettingsValue("Summarize"),
            ChatSettingsKey.FALLBACK_HAPPENED_PREFIX: ChatSettingsValue("[Fallback] "),
        }

        user = createMockUser(userId=456)
        chat = createMockChat(chatId=123, chatType=Chat.GROUP, title="Test Group")
        message = createMockMessage(chatId=123, userId=456, text="/summary")
        message.chat = chat
        message.from_user = user
        message.reply_text = AsyncMock(return_value=message)
    ...
    @pytest.mark.asyncio
    async def testSummaryCommandWithMaxMessagesArg(
        self, summarizationHandler, mockBot, mockDatabase, mockCacheService, mockLlmManager
    ):
        """Test /summary command with maxMessages argument, dood!"""
        summarizationHandler.injectBot(mockBot)
        ...
        message = createMockMessage(chatId=123, userId=456, text="/summary 50")
        message.chat = chat
        message.from_user = user
        message.reply_text = AsyncMock(return_value=message)
        ...
        # Should request 50 messages
        mockDatabase.getChatMessagesSince.assert_called_once()
        callArgs = mockDatabase.getChatMessagesSince.call_args
        assert callArgs[1]["limit"] == 50
...
class TestEdgeCases:
    ...
    @pytest.mark.asyncio
    async def testSummaryCommandDisabledInSettings(self, summarizationHandler, mockBot, mockCacheService):
        """Test /summary command when disabled in settings, dood!"""
        summarizationHandler.injectBot(mockBot)

        mockCacheService.getChatSettings.return_value = {ChatSettingsKey.ALLOW_SUMMARY: ChatSettingsValue("false")}
        ...
        # No assertions needed as it just returns

    @pytest.mark.asyncio
    async def testTopicSummaryCommandInGroupChat(
        self, summarizationHandler, mockBot, mockDatabase, mockCacheService, mockLlmManager
    ):
        """Test /topic_summary command in group chat, dood!"""
        summarizationHandler.injectBot(mockBot)
        ...
        user = createMockUser(userId=456)
        chat = createMockChat(chatId=123, chatType=Chat.SUPERGROUP)
        message = createMockMessage(chatId=123, userId=456, text="/topic_summary")
        message.chat = chat
        message.from_user = user
        message.reply_text = AsyncMock(return_value=message)
        message.message_thread_id = 5  # In a topic
        ...
        # Should call LLM with thread ID
        mockModel = mockLlmManager.getModel("gpt-4")
        mockModel.generateTextWithFallBack.assert_called_once()
